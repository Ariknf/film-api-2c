...
app.put('/movies/:id', [authenticateToken,authorizeRole('admin')], (req, res) => {
    console.log('request UPDATE /movies oleh user:', req.user.username);
    const { title, director, year } = req.body;
...
app.post('/directors', authenticateToken, (req, res) => {
    ...
    if (!name || !birthYear) {
        return res.status(400).json({ error: 'name dan birthYear wajid diisi' })
    }
...
app.put('/directors/:id',[authenticateToken,authorizeRole('admin')], (req, res) => {
     console.log('request UPDATE /directors oleh user:', req.user.username);
    const { name, birthYear } = req.body;
